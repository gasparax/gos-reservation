...
class ReservationManager(Manager):
    ...
    def retrieve_by_customer_id_in_last_14_days(user_id):
        Manager.check_none(user_id=user_id)
        cond1 = db.and_(Reservation.user_id == user_id,
                        Reservation.end_time >= (datetime.utcnow() - timedelta(days=14)))
    ...
    def retrieve_all_contact_reservation_by_id(id_):
        ...
        cond2 = db.and_(Reservation.start_time < pos_reservation.end_time,
                        pos_reservation.end_time < Reservation.end_time)
        cond3 = db.or_(cond1, cond2)
        cond4 = db.and_(cond3, Reservation.restaurant_id == pos_reservation.restaurant_id)
        cond5 = db.and_(cond4, Reservation.user_id != pos_reservation.user_id,
                        Reservation.start_time < datetime.today())  # discard the reservations for the same positive customer
    ...
    @staticmethod
    def retrieve_by_date_time_table(table_id, start_time, end_time):
        Manager.check_none(table_id=table_id)
